...
def get_training_weights(X_train, model):
    ...
    for ts in X_train:
        ts = ts.reshape(1, -1, 1)
        conv_out, predicted = intermediate_model(ts, training=False)
        conv_out, predicted = conv_out.numpy(), predicted.numpy()

        pred_label = np.argmax(predicted)
        cas = np.zeros(shape=(conv_out.shape[1]), dtype=float)
