...
from wildboar.datasets import load_dataset, install_repository, list_datasets, list_repositories
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=["all", "cnn", "lstm", "none", "cnn+lstm", "cnn+none", "lstm+none"],
        help="Which autoencoder method(s) to run."
    )

    parser.add_argument("--contiguity", type=str, default="both",
        choices=["none", "global", "local", "both"],
        help="Where to enforce band contiguity (few contiguous windows).")
    parser.add_argument("--bands-k", type=int, default=3,
        help="Max number of contiguous windows to allow edits in.")
    parser.add_argument("--band-l", type=int, default=48,
        help="Length (timesteps) of each contiguous window.")
    parser.add_argument("--band-min-len", type=int, default=8,
        help="Minimum segment length to consider when building masks.")
    parser.add_argument("--global-percentile", type=float, default=90.0,
        help="Percentile threshold for global interval importances before selecting top-K bands.")
    parser.add_argument("--lam-tv", type=float, default=1e-3,
        help="Total-variation weight to encourage smooth changes inside bands (0 disables).")
    parser.add_argument("--eps-linf", type=float, default=0.25,
        help="Per-timestep |delta| cap in the normalised space (None or negative to disable).")
    ...
    for train_index, test_index in skf.split(X, y_copy):
        ...
        # Get 100 samples for CF evaluation if test size larger than 100
        ...
        fold_idx += 1
        GLOBAL_MASK_FILE = f"masks/global_mask_{A.dataset}_fold{fold_idx}.npy"
        logger.info(
       ...
       # Determine which methods to run
        methods_to_run = []
        if A.method == "all":
            methods_to_run = ["cnn", "lstm", "none"]
        elif "+" in A.method:
            methods_to_run = A.method.split("+")
        else:
            methods_to_run = [A.method]
        ...
        if A.w_type == "global":
            if os.path.exists(GLOBAL_MASK_FILE):
                step_weights_global = np.load(GLOBAL_MASK_FILE)
                print(f"Loaded cached global mask from {GLOBAL_MASK_FILE}")
            else:
                step_weights_global = get_global_weights(
                    X_train_processed_padded,
                    y_train_classes,
                    classifier_model=classifier,
                    random_state=RANDOM_STATE,
                    k_bands=A.bands_k,
                    min_len=A.band_min_len,
                    percentile=A.global_percentile,
                )
                Path(GLOBAL_MASK_FILE).parent.mkdir(parents=True, exist_ok=True)
                np.save(GLOBAL_MASK_FILE, step_weights_global)
                print(f"Saved global mask to {GLOBAL_MASK_FILE}")
        if A.w_type == "global":
            step_weights = step_weights_global
        ...
        if "cnn" in methods_to_run:
            ...
            lr_list = [A.lr_list[0]] if len(A.lr_list) > 0 and A.lr_list[0] is not None else [0.001, 0.0001]
            ...
            np.savez_compressed(
                CF_DIR / f"cf_fold{fold_idx}_cnn.npz",
                x_orig=rand_X_test_original,
                x_cf=best_cf_samples,
                y_true=rand_y_true,      # Ground truth labels
                y_pred=rand_y_pred,      # Original model predictions  
                y_cf=cf_pred_labels,     # CF predictions
            )
        ...
        if "lstm" in methods_to_run:
            ...
            lr_list2 = [A.lr_list[1]] if len(A.lr_list) > 1 and A.lr_list[1] is not None else [0.001, 0.0001]
            ...
            np.savez_compressed(
                CF_DIR / f"cf_fold{fold_idx}_lstm.npz",
                x_orig=rand_X_test_original,
                x_cf=best_cf_samples2,
                y_true=rand_y_true,
                y_pred=rand_y_pred,
                y_cf=cf_pred_labels2,
            )
        ...
        ###############################################
        if "none" in methods_to_run:
            logger.info(
                f"The current prediction margin weight is {pred_margin_weight}, for [No autoencoder]."
            )

            lr_list3 = [A.lr_list[2]] if len(A.lr_list) > 2 and A.lr_list[2] is not None else [0.001, 0.0001]
            ...
            np.savez_compressed(
                CF_DIR / f"cf_fold{fold_idx}_ne.npz",
                x_orig=rand_X_test_original,
                x_cf=best_cf_samples3,
                y_pred=rand_y_pred,
                y_true=rand_y_true,
                y_cf=cf_pred_labels3,
            )
